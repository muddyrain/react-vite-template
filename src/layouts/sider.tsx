...
// 默认展示二级菜单
const handleOpenKeys = (datasource: RoutesProps[] = []): string[] => datasource.map((e) => e.path as string)
...
const Fragment: FC<{
  ...
  return (
    <Sider theme='light' className={`${styles.sider} overflow-y-auto`} collapsed={false}>
      <Menu
        mode='inline'
        defaultOpenKeys={handleOpenKeys(routes)}
        selectedKeys={[(configuration?.parentPath as string) || (configuration?.path as string)]}
        items={menuList}
