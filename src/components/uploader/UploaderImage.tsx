...
const Uploader: FC<UploaderImageProps> = ({ defaultUrl = '/file/uploadFile', returnFormatter, onChange, value }) => {
  useEffect(() => {
    setImageUrl(value)
  }, [value])
  ...
  const handleChange = (info) => {
    ...
    switch (status) {
      ...
      case 'done':
        setLoading(false)
        onChange?.(returnFormatter ? returnFormatter(response, info.file) : response, info.file)
        setImageUrl(returnFormatter ? (returnFormatter(response, info.file) as string) : (response.data as string))
        break
      case 'error':
        setLoading(false)
        break
